...
function makePlayer(socket, data) {
  ...
  return { id: socket.id, name: (data.name || "Piloto").slice(0, 18), color: data.color || "#00e5ff", accountId: socket.accountId || null, pilot: (pilotNum >= 1 && pilotNum <= 16) ? pilotNum : 1, customPhotoUrl: safeAvatarPhotoUrl(data.customPhotoUrl), planeType: VALID_PLANES.includes(data.planeType) ? data.planeType : "cessna", vote: null, team: null, ready: false, state: "alive", health: MAX_HEALTH, shield: 0, kills: 0, deaths: 0, position: { x: 0, y: 0.1, z: 0 }, yaw: 0, pitch: 0, roll: 0, abilityActive: false, abilityTimer: 0, abilityCooldown: 0, invisible: false, invulnerable: false, fieldCooldowns: new Map(), titles: [] };
}
// NOVO — busca os títulos escolhidos pelo jogador (se tiver conta) e
// atualiza `p.titles` assim que a resposta do banco chegar, avisando a
// sala de novo pra quem já estava vendo o lobby também ver o título
// aparecer. Chamado toda vez que um jogador entra numa sala (criada,
// listada ou Sala Livre) — mostrar o título já conquistado não tem
// nada a ver com CONTAR estatística nova, é só exibição.
function attachPlayerTitles(room, p) {
  if (!p.accountId) return;
  profileModule.getPublicTitles(p.accountId, (titles) => {
    p.titles = titles || [];
...
function addPlayer(socket, roomId, playerData) { const room = rooms.get(roomId); if (!room || room.players.length >= room.maxPlayers) return null; const p = makePlayer(socket, playerData); room.players.push(p); if (room.mode === "teams") reassignTeams(room); sockets.set(socket.id, roomId); socket.join(roomId); noteLifeStart(p.id); attachPlayerTitles(room, p); return p; }
...
function startGame(roomId) {
  ...
  io.to(roomId).emit("match-loading", { mode: room.mode, map: room.map, planeMode: room.planeMode, sorteioResult, players: room.players.map(p => ({ id: p.id, name: p.name, color: p.color, planeType: p.planeType, team: p.team, position: p.position, yaw: p.yaw, health: p.health, titles: p.titles || [] })), weather: weatherSnapshot(room) });
...
function endMatch(room, resultMsg, winner) {
  ...
  // PEDIDO: só sala CRIADA conta pra carreira — checkFFAEnd (quem chama
  // endMatch) já nem roda pra Sala Livre (early return por isFreeRoom),
  // então na prática endMatch só acontece em sala criada. A guarda
  // abaixo é só uma segunda trava, caso alguém chame endMatch de outro
  // lugar no futuro sem perceber essa regra.
  if (room.isFreeRoom) { io.to(room.id).emit("match-end", { message: resultMsg, winner, standings: room.players.map(p => ({ name: p.name, kills: p.kills, deaths: p.deaths })) }); setTimeout(() => resetRoom(room), 8000); return; }
  let winnerIds = [];
  if (winner) {
...
function applyDamage(room, targetId, dmg, shooterId, weaponType) {
  ...
  if (target.health <= 0) {
    ...
    // PEDIDO: só conta pra carreira em SALA CRIADA — nunca na Sala Livre.
    // Bots não passam por aqui de jeito nenhum (não existem no servidor).
    if (!room.isFreeRoom) profileModule.recordDeath(target.accountId, dmgTaken, aliveSec);
    const shooter = room.players.find(p => p.id === shooterId);
    if (shooter && shooter.id !== targetId) {
      shooter.kills++;
      if (!room.isFreeRoom) profileModule.recordKill(shooter.accountId, weaponType);
      if (!room.firstBlood) { room.firstBlood = true; if (!room.isFreeRoom) profileModule.recordFirstBlood(shooter.accountId); }
...
io.on("connection", (socket) => {
  ...
  // medalha secreta reportada pelo cliente (Tiro Perfeito) — PEDIDO: só
  // conta em sala criada, igual ao resto da carreira.
  socket.on("report-medal", (key) => {
    const roomId = sockets.get(socket.id); const room = rooms.get(roomId);
    if (!room || room.isFreeRoom) return;
  ...
  socket.on("join-free-room", (playerData, cb) => {
    ...
    attachPlayerTitles(room, p);
    if (cb) cb({ success: true, roomId: room.id });
    broadcastRoom(room.id);
    socket.to(room.id).emit("player-joined", { id: p.id, name: p.name, color: p.color, planeType: p.planeType, team: p.team, position: p.position, yaw: p.yaw, health: p.health, titles: p.titles || [] });
    socket.emit("free-room-enter", { map: room.map, mode: room.mode, players: room.players.map(pl => ({ id: pl.id, name: pl.name, color: pl.color, planeType: pl.planeType, team: pl.team, position: pl.position, yaw: pl.yaw, health: pl.health, titles: pl.titles || [] })), weather: weatherSnapshot(room) });
  });
  ...
  socket.on("blimp-killed", (d) => {
    ...
    // PEDIDO: dirigível abatido na Sala Livre NÃO conta pra carreira —
    // só em sala criada.
    if (!room.isFreeRoom) profileModule.recordBlimpKill(socket.accountId);
