...
function makePlayer(socket, data) {
  ...
  return {
    ...
    // PEDIDO: sistema de títulos — "carteirinha" (patente + até 3 títulos +
    // foto) desse jogador, carregada do banco assim que ele entra numa sala
    // (ver attachPlayerBadge). Começa vazia e é preenchida perguntando pro
    // profile.js — nula pra quem não tem conta, sem quebrar nada.
    badge: { rankKey: null, titles: [], photoUrl: null },
    // PEDIDO: usados só pra estatística real de dano-tomado/tempo-vivo
    // dessa vida (zerados a cada spawn/respawn) — sem isso as medalhas
    // "Blindagem Viva" e "Implacável" nunca calculavam nada de verdade.
    spawnedAt: Date.now(),
    damageTaken: 0,
  };
}

// PEDIDO: busca a patente/títulos/foto reais no banco (profile.js) e
// atualiza a "carteirinha" do jogador — depois manda um room-update pra
// sala inteira ver o resultado (chega um instante depois do jogador
// entrar, é assíncrono, mas normal: ninguém percebe esse delay de
// milissegundos). Sem conta (accountId null) fica com a carteirinha vazia.
function attachPlayerBadge(roomId, playerId) {
  const room = rooms.get(roomId);
  const p = room && room.players.find(pl => pl.id === playerId);
  if (!p || !p.accountId) return;
  profileModule.getPlayerBadge(p.accountId, (badge) => {
    const room2 = rooms.get(roomId);
    const p2 = room2 && room2.players.find(pl => pl.id === playerId);
    if (!p2) return; // já saiu da sala antes da resposta do banco
    p2.badge = badge;
...
function roomState(roomId) {
  ...
  return {
    ...
    players: room.players.map(p => ({
      ...
      // PEDIDO: patente + até 3 títulos + foto — pros amigos/oponentes
      // verem na sala (lobby e placar). Ver attachPlayerBadge().
      rankKey: p.badge ? p.badge.rankKey : null,
      titles: p.badge ? p.badge.titles : [],
      photoUrl: p.badge ? p.badge.photoUrl : null,
    })),
  };
}
...
function resetRoom(room) {
  ...
  room.players.forEach(p => {
    ...
    p.spawnedAt = Date.now(); p.damageTaken = 0;
  });
  if (room.weather) {
...
function beginPlaying(room) {
  ...
  room.firstBloodDone = false; // PEDIDO: título "Primeiro Ataque" — 1 por partida
  room.players.forEach(p => { p.spawnedAt = Date.now(); p.damageTaken = 0; }); // zera pra "Blindagem Viva"/"Implacável"
...
function endMatch(room, resultMsg, winner) {
  ...
  // ignorado silenciosamente dentro de recordMatchResults. ESSA é a
  // chamada que faltava pra vitórias, MVP e "Último no Céu" contarem de
  // verdade (recordMatchPlayed sozinho só somava partida/tempo de voo).
  const now = Date.now();
  const playtimeSeconds = room.matchStartedAt ? (now - room.matchStartedAt) / 1000 : 0;
  // MVP da partida = maior número de abates (precisa ter abatido pelo
  // menos 1 pra valer — sem isso, uma partida sem combate "premiava"
  // qualquer um com 0 abates).
  let mvp = null;
  room.players.forEach(p => { if (p.kills > 0 && (!mvp || p.kills > mvp.kills)) mvp = p; });
  // "Último no Céu": só faz sentido em FFA com mais de 1 jogador — quem
  // sobrou vivo sozinho no fim da partida.
  const survivors = room.players.filter(p => p.state === "alive");
  const lastSurvivor = (room.mode !== "teams" && room.players.length > 1 && survivors.length === 1) ? survivors[0] : null;
  room.players.forEach(p => {
    const isWinner = !!winner && (
      (typeof winner === "string" && p.team === winner) || // modo esquadrões: winner = 'red'/'blue'
      (typeof winner === "object" && winner.id === p.id)   // FFA: winner = { id, name }
    );
    profileModule.recordMatchResults(p.accountId, {
      playtimeSeconds,
      win: isWinner,
      mvp: !!(mvp && mvp.id === p.id),
      lastSurvivor: !!(lastSurvivor && lastSurvivor.id === p.id),
      aliveSeconds: (now - (p.spawnedAt || now)) / 1000,
    });
  });
...
function scheduleFreeRespawn(room, playerId) {
  setTimeout(() => {
    ...
    p.abilityActive = false; p.abilityTimer = 0; p.abilityCooldown = 0;
    p.fieldCooldowns = new Map();
    p.spawnedAt = Date.now(); p.damageTaken = 0; // "Blindagem Viva"/"Implacável" contam por vida
...
function applyDamage(room, targetId, dmg, shooterId, weaponType) {
  ...
  // PEDIDO: acumula dano tomado nesta vida (zera a cada spawn/respawn) —
  // usado só pra medalha "Blindagem Viva" (ver profile.js/evaluateMedals).
  ...
  if (target.health <= 0) {
    ...
    target.health = 0;
    target.deaths++;
    const aliveSeconds = (Date.now() - (target.spawnedAt || Date.now())) / 1000;
    profileModule.recordDeath(target.accountId, target.damageTaken, aliveSeconds);
    target.damageTaken = 0;
    const shooter = room.players.find(p => p.id === shooterId);
    if (shooter && shooter.id !== targetId) {
      shooter.kills++;
      profileModule.recordKill(shooter.accountId);
      // PEDIDO: "Primeiro Ataque" — só o primeiríssimo abate de cada
      // partida conta (room.firstBloodDone é resetado em beginPlaying).
      if (!room.firstBloodDone) {
        room.firstBloodDone = true;
        profileModule.recordFirstBlood(shooter.accountId);
      }
...
io.on("connection", (socket) => {
  ...
  socket.on("create-room", (data, cb) => {
    ...
    broadcastRoom(room.id);
    attachPlayerBadge(room.id, p.id);
  });
  ...
  socket.on("join-room", (data, cb) => {
    ...
    attachPlayerBadge(data.roomId, p.id);
  });
  ...
  // PEDIDO: medalhas SECRETAS reportadas pelo próprio cliente (ex: Tiro
  // Perfeito — 20 acertos seguidos, ver social-client.js). O servidor só
  // aceita chaves que estão na lista SECRET_MEDALS (ver profile.js); tudo
  // mais é ignorado silenciosamente. Sem login (accountId null) não faz
  // nada — combate sem conta continua funcionando normal, só não conta
  // pra carreira militar.
  socket.on("report-medal", (key) => {
    if (!socket.accountId) return;
    profileModule.unlockSecret(socket.accountId, key);
  });
