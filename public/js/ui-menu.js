...
//  (img/piloto1.png até img/piloto15.png) + 1 opção extra (16) que só
//  existe pra quem está logado e já enviou uma foto própria (ver
//  social-client.js/setCustomPilotPhoto) — pra quem não tem conta ou
//  ainda não subiu foto, o carrossel continua com exatamente 15
//  opções, sem slot vazio.
// ================================================================
let PILOT_COUNT = 15;
let customPilotPhotoUrl = null;

function pilotImagePath(index) {
    if (index === 16 && customPilotPhotoUrl) return customPilotPhotoUrl;
    return 'img/piloto' + index + '.png';
}

// Chamado por social-client.js quando a conta já tem foto salva (ao
// logar) ou quando uma foto nova acaba de ser enviada. `autoSelect`
// true = pula na hora pro piloto 16 (pedido explícito: ao trocar a
// foto, ela já aparece selecionada); false = só disponibiliza a opção
// sem mexer no piloto que já estava selecionado (usado ao logar, pra
// não sobrescrever a escolha salva do jogador).
function setCustomPilotPhoto(url, autoSelect) {
    customPilotPhotoUrl = url;
    PILOT_COUNT = 16;

    if (autoSelect) selectPilotIndex(16);
    else renderPilotCarousel();
}
window.setCustomPilotPhoto = setCustomPilotPhoto;
...
function renderPilotCarousel() {
    ...
    if (label) label.textContent = (selectedPilotIndex === 16 && customPilotPhotoUrl) ? 'Sua foto' : ('Piloto ' + selectedPilotIndex);
}

// ================================================================
//  MUDANÇA CRÍTICA: salva o piloto escolhido na CONTA.
//  Sem essa linha, o servidor nunca sabia qual piloto você escolheu,
//  e ao recarregar ele via "preferredPilot = null" e forçava a foto.
// ================================================================
function selectPilotIndex(index) {
    ...
    if (typeof socialSaveSettings === 'function') {
        socialSaveSettings({ preferredPilot: selectedPilotIndex });
    }
}
...
function selectCarouselIndex(prefix, scope, index) {
    ...
    // PEDIDO: avião preferido persistido na CONTA (ver social-client.js) —
    // só quando a escolha é feita no carrossel do MENU PRINCIPAL (as
    // outras telas são escolhas pontuais pra aquela partida/lobby, não
    // uma preferência geral).
    if (prefix === 'menu' && typeof socialSaveSettings === 'function') {
        socialSaveSettings({ preferredPlane: type });
    }
}
...
// ================================================================
//  MUDANÇA CRÍTICA: salva música na CONTA, se logado.
// ================================================================
function setLobbyMusicEnabled(on) {
    ...
    if (typeof socialSaveSettings === 'function') {
        socialSaveSettings({ musicEnabled: lobbyMusicEnabled });
    }
}

// ================================================================
//  MUDANÇA CRÍTICA: salva som na CONTA, se logado.
// ================================================================
function setGameSoundMuted(on) {
    ...
    if (typeof socialSaveSettings === 'function') {
        socialSaveSettings({ soundEnabled: !gameSoundMuted });
    }
}
...
document.getElementById('modal-create-confirm').addEventListener('click', () => {
    ...
    window.__pendingCreateRoom = {
        ...
        playerData: {
            ...
            customPhotoUrl: (selectedPilotIndex === 16 ? customPilotPhotoUrl : null)
        }
    };
...
function refreshRoomList() {
    ...
    tempSocket.on('connect', () => {
        tempSocket.emit('list-rooms', (list) => {
            ...
            if (!list.length) {
                container.innerHTML = '<div style="text-align:center;color:#7fbfd6;opacity:0.5;padding:20px;font-size:13px;">Nenhuma sala aberta.</div>';
...
function bindHostSettingsGrid(containerId, dataAttr, eventKey) {
    ...
    el.addEventListener('click', (e) => {
        ...
        const payload = {};
        payload[eventKey] = btn.dataset[dataAttr];
...
document.getElementById('controls-reset-default').addEventListener('click', () => {
